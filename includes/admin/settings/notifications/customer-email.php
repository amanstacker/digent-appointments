...
function dgap_register_customer_email_notifications_section() {

	add_settings_section(
		'dgap_notifications_customer_email',
		esc_html__( 'Customer Email', 'digent-appointments' ),
		'dgap_customer_email_notifications_section_callback',
		'dgap_notifications_customer_email'
	);
}
add_action( 'admin_init', 'dgap_register_customer_email_notifications_section' );


function dgap_customer_email_notifications_section_callback() {
    ...
    $default_templates  =   isset( $email_templates['customer_email_templates'] ) ? $email_templates['customer_email_templates'] : [];
	...
	foreach ( $statuses as $status => $label ) :
		...
		$option_key = "customer_email_template_{$status}";
		$content    = $options[ $option_key ] ?? $default_templates[ $status ];

        $checked    = isset( $options["customer_email_{$status}_enabled"] ) ? $options["customer_email_{$status}_enabled"] : 1;
		?>
		...
		<div class="dgap-accordion">
			<h4 class="dgap-accordion-title">
                ...
                <label class="dgap-email-labels dgap-switch">
                    <input type="hidden"
                        name="dgap_notifications_settings[customer_email_<?php echo esc_attr($status); ?>_enabled]"
                        value="0">
                    <input type="checkbox"
                        name="dgap_notifications_settings[customer_email_<?php echo esc_attr($status); ?>_enabled]"
                        value="1"
